...
\begin{itemize}
    \item \( \ket{s}_Q \otimes \ket{t}_R = \ket{(s, t)}_{(Q, R)} \)
    \item \( {O_1}_{Q} \cdot {O_2}_{(Q,R)} = ((O_1 \otimes \mathbf{1}_\mathcal{O}) \cdot O_2)_{(Q,R)} \)
    \item \( M_{r_1} \sum_{i} \ket{(i, i)}_{(r_1, r_2)} = M^T_{r_2} \sum_{i}\ket{(i, i)}_{(r_1, r_2)} \)
    \item \( \bra{\Phi}_{(x, y)} M_y \cdot N_x \ket{\Phi}_{(x, y)} = \mathrm{tr}(M^T N) \), where $\ket{\Phi} = \sum_{i} \ket{(i, i)}$
    \item \( \sum_{i} {}_x\bra{i} O_x \ket{i}_x = \mathrm{tr}(O) \)
    \item \( \sum_{j} {}_y\bra{j} (\sum_{i} {}_x\bra{i} O_{(x,y)} \ket{j}_x) \ket{j}_y = \mathrm{tr}(O) \)
    \item \( \sum_{i} {}_{(x, y)}\bra{i} O_{(x,y)} \ket{i}_{(x, y)} = \mathrm{tr}(O) \)
    \item \( \mathrm{tr}_x(\mathrm{tr}_y(O_{((y,z),x)})) = \mathrm{tr}_y(\mathrm{tr}_x(O_{((y,z),x)})) \)
    \item \( \mathrm{tr}_x(\mathrm{tr}_y(O_{((y,z),x)})) = \mathrm{tr}_{(x, y)}(O_{((y,z),x)}) \)
    \item \begin{align*}
    ...
    \item     
        ...
        \end{align*}
    \item \( \ket{i}_{a;b}\bra{j} \cdot C_{(b,c)} \cdot D_{(c,d)} = {}_b\bra{j} \cdot C_{(b,c)} \cdot D_{(c,d)} \cdot \ket{i}_{a}\)
    \item \( (A_{(a,b)} \otimes B_{(c,d)} \otimes C_{(e,f)}) \cdot (D_{(b,c)} \otimes E_{(d,e)}) \cdot (F_{(a,b)} \otimes G_{(c,d)} \otimes H_{(e,f)}) = (A_{(a,b)} \otimes C_{(e,f)}) \cdot (B_{(c,d)} \cdot D_{(b,c)}) \cdot (E_{(d,e)} \cdot G_{(c,d)}) \cdot (F_{(a,b)} \otimes H_{(e,f)})\)
\end{itemize}