...
\begin{itemize}
    ...
    \[
        ...
        \frac{\WF(E)[\Gamma]}{E[\Gamma] \vdash \mathsf{Bit} : \Index}
    \]
    ...
    \[  \textbf{Basis-0} \qquad
        \frac{\WF(E[\Gamma])}{E[\Gamma] \vdash 0 : \Basis(\mathsf{Bit})}
    \]
    \[  \textbf{Basis-1} \qquad
        \frac{\WF(E[\Gamma])}{E[\Gamma] \vdash 1 : \Basis(\mathsf{Bit})}
    \]
    \[
    ...
    \[
        ...
        {E[\Gamma] \vdash \tD_1\otimes\tD_2 : \DType(s_1\cup s_2,s_1'\cup s_2')}
    \]
    \[
        \textbf{L-Dot}\qquad
        \frac{
...
\begin{ruletable}{Rules for Bit index.}
    %
    R-BIT-DELTA
    & $\delta_{0, 1} \ \reduce\ 0$ \\
    %
    R-BIT-ONEO
    & $\ONEO(\textsf{Bit})\ \reduce\ \ket{0}\bra{0} + \ket{1}\bra{1} $ \\
    %
    R-BIT-SUM
    & $\sum_{i \in \mathbf{U}(\textsf{Bit})} A\ \reduce\ A\{i/0\} + A\{i/1\}$
\end{ruletable}
