...
The typing system is responsible for classifying terms within a proof system, according to the types of variables and definitions. We use an environment \( E \) and context \( \Gamma \) for typing. \( E \) includes definitions and assumptions, and the context \( \Gamma \) tracks bound variables during the proof. Both \( E \) and \( \Gamma \) consist of sequences of assumptions \( x : T \) or definitions \( x := t : T \).
...
We say an expression \( t \) has type \( X \) in context \( E[\Gamma] \) if the typing judgment \( E[\Gamma] \vdash t : X \) can be proven through the rules in~\Cref{sec: full typing rules}. 
It involves maintaining a well-formed environment and context \( E[\Gamma] \), which is built incrementally:
\[
    \frac{}{\WF([])[]}
    \qquad
    \frac{\WF(E)[] \qquad x \notin E}{\WF(E; x : \Index)[]}
...
\]
Starting with an empty context, new index symbols can be introduced, and symbols with checked types can be defined. 
Typing judgments are then made inductively using the information from \( E[\Gamma] \). For instance, the rule \( x : \Index \in E[\Gamma] \) holds if \( E \) or \( \Gamma \) contains an assumption for \( x \).
...
The idea behind the interpretation of types and terms is to formalize the typing relation using set-theoretic inclusion. Specifically, for a well-formed context \( E[\Gamma] \), term \( t \), and type \( T \), if \( E[\Gamma] \vdash t : T \), then for any valuation \( v \), the semantics of \( t \) must belong to the semantics of \( T \).
...
With these syntax, typing and semantics introduced, we demonstrate the formalization of the symbols used in the motivating example to conclude this section.
